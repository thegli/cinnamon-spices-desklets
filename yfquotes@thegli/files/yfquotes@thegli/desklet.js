        ...
        if (soupMessage) {
            if (IS_SOUP_2) {
                ...
                try {
                    ...
                    this.logDebug(`get_status() exception: ${err}`);
...
YahooFinanceQuoteReader.prototype = {
    ...
    retrieveCrumbWithCurl(networkSettings, callback) {
        ...
        if (authCookie) {
            ...
            this.quoteUtils.logDebug(`Curl retrieveCrumb arguments: ${JSON.stringify(curlArgs)}`);
            ...
            let subProcess = spawnCommandLineAsyncIO("", (stdout, stderr, exitCode) => {
                ...
                try {
                    ...
                    _that.quoteUtils.logDebug("Curl retrieveCrumb - Failed to send signal to subprocess: " + err);
                }
            }, { argv: curlArgs });
        } else {
    ...
    retrieveFinanceDataWithCurl(requestUrl, networkSettings, callback) {
        ...
        this.quoteUtils.logDebug(`Curl retrieveFinanceData arguments: ${JSON.stringify(curlArgs)}`);
        ...
        let subProcess = spawnCommandLineAsyncIO("", (stdout, stderr, exitCode) => {
            ...
            try {
                ...
                _that.quoteUtils.logDebug(`Curl retrieveFinanceData - Failed to send signal to subprocess: ${err}`);
            }
        }, { argv: curlArgs });
    },
    ...
    retrieveFinanceDataWithSoup2(requestUrl, networkSettings, callback) {
        ...
        this.httpSession.queue_message(requestMessage, (session, message) => {
            ...
            if (_that.quoteUtils.isOkStatus(message)) {
                ...
                _that.quoteUtils.logDebug("Soup2 Current authorization parameters have expired. Discarding them.");
                _that.dropAuthParams();
                callback.call(_that, _that.buildErrorResponse(_("Authorization parameters have expired")), true, true);
            } else {
    ...
    retrieveFinanceDataWithSoup3(requestUrl, networkSettings, callback) {
        ...
        this.httpSession.send_and_read_async(message, Soup.MessagePriority.NORMAL, null, (session, result) => {
            ...
            if (_that.quoteUtils.isOkStatus(message)) {
                ...
                _that.quoteUtils.logDebug("Soup3 Current authorization parameters have expired. Discarding them.");
                _that.dropAuthParams();
                callback.call(_that, _that.buildErrorResponse(_("Authorization parameters have expired")), true, true);
            } else {
    ...
    createYahooQueryUrl(quoteSymbolsArg, crumb) {
        const queryUrl = `${YF_QUOTE_URL}?lang=en-US&region=US&formatted=false&symbols=${encodeURIComponent(quoteSymbolsArg)}&crumb=${crumb}`;
    ...
    createQuoteLabel(labelText, symbolCustomization, width, settings) {
        ...
        if (settings.linkQuote) {
            ...
            symbolButton.connect("clicked", () => {
                Gio.app_info_launch_default_for_uri(YF_QUOTE_WEBPAGE_URL + symbolCustomization.symbol, global.create_app_launch_context());
            });
            return symbolButton;
        } else {
            return label;
